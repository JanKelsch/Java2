...
public class StreamHWTest {
    ...
    @Test // Done (not Integer but Double in List of "squares")
    public void shouldReturnSquareRoot() {
        List<Integer> numbers = asList(1, 4, 16, 256);
        List<Double> squares = StreamHW.returnSquareRoot(numbers);
        assertEquals(squares, asList(1.0, 2.0, 4.0, 16.0));
    }
    ...
    @Test// Done (not Integer but long in "count" in method "countUsersOlderThen25")
    public void shouldReturnNumberOfUsersOlderThen25() {
        List<User> users = User.getUsersWithAge(18, 20, 21, 22, 23, 24, 25, 26);
        long count = StreamHW.countUsersOlderThen25(users);
        assertTrue(count == 1);
    }
