...
public class Demo {
	...
	public static void main(String[] args) {
		...
		List<Address> getAddress = persons
				.stream()
		...
		List<String> noDublicates = names
				.stream()
				.distinct()
		...
		Map<Integer, List<String>> groupAges = persons2
				.stream().collect(
				Collectors.groupingBy(Person::getAge, Collectors
		...
		boolean isDigit = s2
				.chars().allMatch(Character::isDigit); // #7
		System.out.println("7) Is digital string: " + isDigit);

		int sumOfAges = persons2
				.stream()
				.filter(i -> i.getAge() > 17)
				.map(Person::getAge)
				.reduce(0, (a, b) -> a + b); // #10
